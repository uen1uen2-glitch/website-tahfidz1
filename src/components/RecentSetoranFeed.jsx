  ...
  const feedItems = [
    {
      ...
      time: 'Today',
      badges: [
        { text: 'Sangat Lancar', className: 'bg-[#e6f4ea] text-[#137333]' },
        { text: 'Ziadah', className: 'bg-surface-variant text-on-surface-variant' },
      ],
    ...
    },
    {
      ...
      time: 'Yesterday',
      badges: [
        { text: 'Lancar', className: 'bg-[#fff8e1] text-[#f57f17]' },
        { text: "Muraja'ah", className: 'bg-surface-variant text-on-surface-variant' },
      ],
    ...
    },
    {
      ...
      badges: [
        { text: 'Sangat Lancar', className: 'bg-[#e6f4ea] text-[#137333]' },
        { text: 'Ujian Juz', className: 'bg-surface-variant text-on-surface-variant' },
      ],
    ...
    <div className="md:col-span-4 bg-surface-container-lowest border border-outline-variant rounded-xl p-6 shadow-sm flex flex-col h-[400px]">
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-6">
        {feedItems.map((item, idx) => (
          <div
            key={idx}
            className="relative pl-6 border-l-2 pb-2 transition-colors"
            style={{ borderColor: item.active ? 'var(--color-primary-container)' : 'var(--color-surface-variant)' }}
          >
            <div
              className="absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-white transition-colors"
            ...
            <div className="mb-2">
              {item.badges.map((badge, bIdx) => (
                <span key={bIdx} className={`inline-block px-2 py-0.5 rounded-full font-label-md text-[10px] ${badge.className} mr-1`}>
                  {badge.text}
                </span>
              ))}
            </div>
